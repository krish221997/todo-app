...
class CompletedTodos extends Component {
    ...
    render() {
        ...
        const {partially_completed} = todosReducer;
        const {todos} = partially_completed;

        this.rightOpenValue = -Dimensions.get('window').width;

        return (
            <View style={commonStyles.container}>
                {<Title title="Partially Completed List!"/>}
                <FlatList
                    data={todos}
                    ...
                    renderItem={({item, index}) => (
                        <SwipeView
                            ...
                            disableSwipeToRight={config.constants.archived_todos_screen.disable_right_swipe}
                            disableSwipeToLeft={config.constants.archived_todos_screen.disable_left_swipe}
                        />
                    )}
                />
            </View>
